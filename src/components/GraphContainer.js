...
const GraphContainer = ({ nodes }) => {
  ...
  const topLeft = { x: left.x - padding, y: top.y - padding + yFactor}
  const topRight = { x: right.x + padding, y: top.y - padding + yFactor}
  const bottomLeft = { x: left.x - padding, y: bottom.y + padding + yFactor}
  const bottomRight = { x: right.x + padding, y: bottom.y + padding + yFactor}
